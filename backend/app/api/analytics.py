...
from app.db.models import FpsRiskScore, GrievanceCase
...
async def overview(db: Session = Depends(get_db)):
    tot = db.query(GrievanceCase).count()
    op = db.query(GrievanceCase).filter(GrievanceCase.status == 'open').count()
    res = db.query(GrievanceCase).filter(GrievanceCase.status == 'resolved').count()
    top = db.query(FpsRiskScore).order_by(FpsRiskScore.composite_risk_score.desc()).limit(5).all()
    return {
        "success": True, 
        "data": {
            "stats": {
            ...
            "top_risky_fps": [
                {
                    "code": f.fps_code, 
                    "name": f.fps_name, 
                    "score": float(f.composite_risk_score), 
                    "tier": f.risk_tier
                } for f in top
            ]
        }
...
async def fps_risk(dist: Optional[str] = None, tier: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    q = q.filter(FpsRiskScore.district_code == dist) if dist else q
    q = q.filter(FpsRiskScore.risk_tier == tier) if tier else q
...
async def trends(fps: Optional[str] = None, db: Session = Depends(get_db)):
    six = datetime.now() - timedelta(days=180)
    q = db.query(func.date_trunc('month', GrievanceCase.created_at), func.count(GrievanceCase.id)).filter(GrievanceCase.created_at >= six)
    q = q.filter(GrievanceCase.fps_code == fps) if fps else q
    r = q.group_by(1).order_by(1).all()
