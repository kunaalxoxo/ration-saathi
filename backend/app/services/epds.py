...
class RationCardDetails(BaseModel):
    card_number: str; head_name: str; total_members: int; category: str; fps_code: str; is_active: bool

class AllocationDetails(BaseModel):
    card_number: str; month_year: date; rice_kg: float; wheat_kg: float; sugar_kg: float

class MockEPDSClient:
    def __init__(self):
        try:
            with open(os.path.join("mock_data", "epds_fixtures.json"), "r") as f: self.data = json.load(f)
        except: self.data = {"cards": [], "allocations": []}
